		...
		while(!pilots.isEmpty())
		{
			for(int i=0;i<this.no_pilots;i++)
			{
				...
				if(millis >= ((this.pilots.get(i).get_car_details().get_c_speed()==0)?0:(long)1000/this.pilots.get(i).get_car_details().get_c_speed()))
				{
					...
					float speed = this.race_map.map_data((int)this.pilot_co.get(i).x, (int)this.pilot_co.get(i).y,1);
					int tile = (int)this.race_map.map_data((int)this.pilot_co.get(i).x, (int)this.pilot_co.get(i).y, 0);
